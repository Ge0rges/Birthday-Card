...
class ViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		DispatchQueue.main.async {
			let flowerDimension:CGFloat = 100
			let frameWidth:CGFloat  = self.view.frame.width
			let frameHeight:CGFloat = self.view.frame.height
			let flowerFrame:CGRect = CGRect(x: frameWidth/2 - flowerDimension/2, y: frameHeight/4, width: flowerDimension, height: flowerDimension)
